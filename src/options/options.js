 ...
 */
 const enableNecessaryCheckboxIfPaused = async function(pauseState) {
    ...
    if (!pauseState) {
        let policy = await getStorageValue(chrome.storage.sync, "ext_userpolicy")
        if (policy[0] !== true){
            policy[0] = true;
...
const setupSettingsPage = async function() {
    ...
    restoreExceptionList("ext_exglobal", "website_exceptions");
    restoreExceptionList("ext_exfunc", "functional_exceptions");
    restoreExceptionList("ext_exanal", "analytics_exceptions");
    restoreExceptionList("ext_exadvert", "advertising_exceptions");
    restoreExceptionList("ext_knowncookies", "predef-paths");

    let hconsent = await getStorageValue(chrome.storage.sync, "ext_hconsent");
    ...
    let pauseState = await getStorageValue(chrome.storage.local, "ext_pause");
    pauseCheckbox.checked = pauseState;
    enableNecessaryCheckboxIfPaused(pauseState);

    let permScale = await getStorageValue(chrome.storage.sync, "ext_pscale");
    nfslider.value = permScale;
    sliderValDisplay.textContent = permScale;

    internalCheckbox.checked = await getStorageValue(chrome.storage.sync, "ext_useinternal");
...
const updateSelectionsOnStorageChanged = function(changes, area) {
    ...
    if (area === "sync") {
        ...
        if (changedItems.includes("ext_exglobal")) {
            emptyAndRestoreList(changes["ext_exglobal"].newValue, "ext_exglobal", "website_exceptions");
        }

        // update the necessary bias scale
        if (changedItems.includes("ext_pscale")) {
            nfslider.value = changes["ext_pscale"].newValue;
            sliderValDisplay.textContent = changes["ext_pscale"].newValue;
        }

        // update the history consent toggle
        if (changedItems.includes("ext_hconsent")) {
            histCheckbox.checked = changes["ext_hconsent"].newValue;
            statisticsSection.hidden = !changes["ext_hconsent"].newValue;
        }
    } else if (area === "local") {
        // update the pause button
        if (changedItems.includes("ext_pause")){
            pauseCheckbox.checked = changes["ext_pause"].newValue;
            enableNecessaryCheckboxIfPaused(changes["ext_pause"].newValue);
        }
    }
}
...
addExcClickListener("website_excepts_submit", "website_excepts_input", "ext_exglobal", "website_exceptions", "wexcepts-error");
addExcClickListener("func_excepts_submit", "func_excepts_input", "ext_exfunc", "functional_exceptions", "func-error");
addExcClickListener("analytics_excepts_submit", "analytics_excepts_input", "ext_exanal", "analytics_exceptions", "analytics-error");
addExcClickListener("advert_excepts_submit", "advert_excepts_input", "ext_exadvert", "advertising_exceptions", "advert-error");
...
nfslider.addEventListener("mouseup", function(ev) {
    setStorageValue( this.value, chrome.storage.sync, "ext_pscale");
});
...
cconfigButton.addEventListener("click", (ev) => {
    chrome.tabs.create({"active": true, "url": "/options/cconfig.html"});
});
...
document.getElementById("predef-submit").addEventListener("click", async (e) => {
    ...
    let pathList = await getStorageValue(chrome.storage.sync, "ext_knowncookies");
        ...
        try {
            ...
            getExtensionFile(pathString, "json", (result) => {
                if (typeof result === "object" && result !== null && ("name_match" in result || "domain_match" in result || "domain_regex" in result)) {
                    ...
                    setStorageValue(pathList, chrome.storage.sync, "ext_knowncookies");
                    appendItemToList(pathString, "predef-paths", "ext_knowncookies");
                    iElem.value = "";
                    iElem.style.color = "black";
                    errorElem.hidden = true;
                } else {
...
internalCheckbox.addEventListener("click", (ev) => {
    setStorageValue( internalCheckbox.checked, chrome.storage.sync, "ext_useinternal");
});
