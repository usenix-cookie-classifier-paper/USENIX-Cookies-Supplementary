...
const popupSetup = async function() {
    ...
    if (!await getStorageValue(chrome.storage.sync, "ext_hconsent")){
        configButton.disabled = true;
        configButton.style.opacity = "0.5";
    }

    pauseCheckbox.checked = await getStorageValue(chrome.storage.local, "ext_pause");
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
        ...
        if (currentURL.match(ignoredPages)){
            ...
            if (sanitizedDomain) {
                let exglobal = await getStorageValue(chrome.storage.sync, "ext_exglobal");
...
const addGlobalException = async function() {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
        ...
        let sanitizedDomain = urlToUniformDomain(new URL(currentURL).hostname);
        try {
            let domainList = await getStorageValue(chrome.storage.sync, "ext_exglobal");
            ...
            await setStorageValue(domainList, chrome.storage.sync, "ext_exglobal");
        } catch (error) {
...
pauseCheckbox.addEventListener("click", async () => {
    ...
    setStorageValue(pauseStatus, chrome.storage.local, "ext_pause");
});
